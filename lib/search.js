...
async function idFetch(options) {
  ...
  try {
    ...
    while (
      (searchIds.length < results) &&
      ((searchIds.length + 25 * (closestpage - 1)) < (options.count + options.offset))) {
      const query = options.mirror +
        "/search.php?&req=" +
        encodeURIComponent(options.query) +
        // important that view=detailed so we can get the real IDs
...
module.exports = async function (options) {
  try {
    ...
    try {
      ...
      if (data.ids.length <= options.count && data.ids.length == data.count) {
          const sort = options.sort_by || "def"
          const sortmode = (options.reverse ? "DESC" : "ASC")
          if (sort != 'def') {
            parsedResponse.sort(sortResponseData(sort, sortmode))
          }
      }
      return {results: parsedResponse, count: data.count}
    } catch (err) {
      return err
    }
  } catch (err) {
...
function sortResponseData(key, order = 'ASC') {
  return function innerSort(a, b) {
    ...
    if (key == 'year' || key == 'pages' || key == 'filesize') {
      varA = parseInt(a[key]);
      varB = parseInt(b[key]);
    }else{
